import { findOrCreateParentUid, testableToRoamDateUid, testableToRoamDate } from "../src/find-or-create-parent-uid"
...
test("it queries the roamAPI with the roam date", () => {
  ...
  expect(querySpy).toHaveBeenCalledWith(`[:find (pull ?e [* {:block/children [*]}]) :where [?e :node/title "${roamDate}"]]`)
...
test("it searches for the parent block", () => {
  const querySpy = jest.fn()
  querySpy.mockReturnValue([[{uid: expectedUid, children: [{string: parentBlock, uid: expectedParentUid}]}]])
...
test("it creates the daily page if needed", () => {
  ...
  const result = findOrCreateParentUid(date, parentBlock, api, jest.fn())
  expect(createPageSpy).toHaveBeenCalledWith({"page": {"title": "May 18th, 2021", "uid": testableToRoamDateUid(date)}})
})
...
test("it creates the parent block if it does not exist", () => {
  ...
  const result = findOrCreateParentUid(date, parentBlock, api, createBlockSpy)
  expect(createBlockSpy).toHaveBeenCalledWith({"node": {"children": [], "text": parentBlock}, "order": 0, "parentUid": expectedUid})
})